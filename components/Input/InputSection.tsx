...
import { firebaseStorage, firestore } from "../../services/init_firebase";
...
import { LinkOutlined, PictureOutlined, SendOutlined, SmileFilled, SmileOutlined } from "@ant-design/icons";
...
const InputSection: FC<InputSectionProps> = ({
  ...
  return (
    <>
        ...
        <div className="border-dark-lighten flex h-[76px] justify-between border-t p-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <p>
                Replying
                {authUser?.uid === replyInfo.sender ? " to yourself" : ""}
              </p>
            </div>
            {replyInfo.type === "text" ? (
      ...
      <div
        className={`border-dark-lighten flex h-16 items-stretch gap-1 border-t px-4 ${disabled ? "pointer-events-none select-none" : ""
          }`}
      >
        <button
        ...
        >
          <PictureOutlined className="text-2xl" style={{ color: configs.primaryColor }} />
        </button>
        <input
          ref={imageInputRef}
          hidden
        ...
        <button
          ...
          className="text-primary flex flex-shrink-0 items-center text-2xl"
        >
          <LinkOutlined className="text-2xl" style={{ color: configs.primaryColor }} />
        </button>
        <input
          ref={fileInputRef}
          hidden
        ...
        <div className="relative flex flex-shrink-0 items-center">
          {isStickerPickerOpened && (
            <StickerPicker
              isStickerPickerOpen={isStickerPickerOpened}
          ...
          <button
            onClick={() => setIsStickerPickerOpened(!isStickerPickerOpened)}
        ...
        <div className="relative flex flex-shrink-0 items-center">
          {isGifPickerOpened && (
            <GifPicker isGifPickerOpen={isGifPickerOpened} setIsOpened={setIsGifPickerOpened} onSelect={sendGif} />
          )}
        ...
        >
          <div className="relative flex flex-grow items-center">
            <Input
              maxLength={1000}
              disabled={disabled}
              ref={textInputRef as unknown as Ref<InputRef>}
              value={inputValue}
              onChange={(e) => {
            ...
            <button
              type="button"
              onClick={() => setIsIconPickerOpened(!isIconPickerOpened)}
              className="absolute right-2 top-1/2 -translate-y-1/2"
            >
              <SmileFilled className="text-2xl" style={{ color: configs.primaryColor }} />
            </button>

            {isIconPickerOpened && (
              <ClickAwayListener
                onClickAway={() => setIsIconPickerOpened(!isIconPickerOpened)}
              >
                {(ref) => (
